...
func (m StepListModel) renderStep(step Step) string {
	...
	switch step.Status {
	case StepPending:
		prefix = tui.IconPending + " "
		style = tui.MutedStyle
	case StepRunning:
		prefix = tui.RunningStyle.Render(tui.IconRunning) + " "
		style = lipgloss.NewStyle().Foreground(tui.ColorInfo)
	case StepCompleted:
		prefix = tui.SuccessStyle.Render(tui.IconSuccess) + " "
		style = lipgloss.NewStyle().Foreground(tui.ColorSuccess)
	case StepFailed:
		prefix = tui.ErrorStyle.Render(tui.IconError) + " "
