...
EOF
    ...
    if [ -n "$GIT_ASKPASS" ]; then
        echo "GIT_ASKPASS='${GIT_ASKPASS}'" >> "$env_file"
    fi

    if [ -n "$E2E_BINARY_SOURCE" ]; then
        echo "E2E_BINARY_SOURCE='${E2E_BINARY_SOURCE}'" >> "$env_file"
    fi

    echo "DEVNET_HOME='${DEVNET_HOME}'" >> "$env_file"
    echo "GOPRIVATE='github.com/*'" >> "$env_file"
