...
ASKPASS_EOF
        ...
        success "GitHub authentication configured (token + askpass)"
        return 0
    fi
...
main() {
    ...
    # Export environment variables to file for Makefile to source
    local env_file=".e2e-test.env"
    cat > "$env_file" <<EOF
...
EOF
    ...
    if [ -n "$GIT_ASKPASS" ]; then
        echo "export GIT_ASKPASS='${GIT_ASKPASS}'" >> "$env_file"
    fi

    if [ -n "$E2E_BINARY_SOURCE" ]; then
        echo "export E2E_BINARY_SOURCE='${E2E_BINARY_SOURCE}'" >> "$env_file"
    fi

    echo "export DEVNET_HOME='${DEVNET_HOME}'" >> "$env_file"
    echo "export GOPRIVATE='github.com/*'" >> "$env_file"

    info "Environment variables exported to: $env_file"
}
