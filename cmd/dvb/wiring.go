...
func (a *nodeInitializerAdapter) CreateAccountKey(ctx context.Context, keyringDir, keyName string) (*ports.AccountKeyInfo, error) {
	...
	// Parse JSON output - format depends on cosmos-sdk version
	// For now return basic info
	return &ports.AccountKeyInfo{
		Name: keyName,
	}, parseKeyOutput(output)
}
...
// parseKeyOutput is a helper to parse key command output (placeholder for now)
func parseKeyOutput(output []byte) error {
	// TODO: Parse JSON output and populate AccountKeyInfo fields
	_ = output
	return nil
}
