...
func runVersions(cmd *cobra.Command, args []string) error {
	...
	if versionsCacheInfo {
		return showCacheInfo(cacheManager)
	}
	...
	if versionsList || versionsRefresh {
		return listVersions(ctx, client, cacheManager, logger)
	}
...
func listVersions(ctx context.Context, client *github.Client, cacheManager *github.CacheManager, logger *output.Logger) error {
	// Fetch versions
	...
	if fromCache {
		...
		if info != nil {
			...
			fmt.Printf("Source: Cache (fetched %s ago)\n", age)
		}
	} else {
...
func showCacheInfo(cacheManager *github.CacheManager) error {
